        ...
        var rules_RU = function () {
            return rules_basicRU().concat([
                ...
                new Rule('Убираем окончания числительных', function (text) {
                    return  text
                      .replace(new RegExp('(\\d)-я(?=(\\s+[^\\s]+(?:-я|ая|ья|яя|яся))*\\s+(улица|набережная|дорога|линия|аллея|площадь|просека|автодорога|эстакада|магистраль|дамба|хорда|коса|деревня|переправа|площадка|дорожка|трасса)(\\s|$))', 'g'), '$1')

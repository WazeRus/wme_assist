        ...
        var rules_RU = function () {
            return rules_basicRU().concat([
                new Rule('Incorrect status position', function (text) {
                    ...
                    var exW = 'Нехая|Тукая|Мая|Барклая|Батырая|Маклая|Бикбая|Амантая|Нечая|Эшпая|Орая|Прикамья|Алтая|Ухсая|Хузангая|Галлая|Николая|Гая|Эркая|Камая|Пченушая|Здоровья|Палантая|Ярвенпяя|Гулая|Заполярья|Крылья|Приморья|Калина Красная|Краснолесья|Мазая|Умырзая';
                    ...
                    if ( new RegExp('(^|\\s)(' + wStatus + ')(\\s|$)').test(text) ) {
                        ...
                        text = text.replace(new RegExp('(\\d+-я)\\s+(' + wStatus + ')\\s+([^\\s]+(?:лка|ель|аза|йка))$'), '$2 $1 $3');
                    } else
                ...
                new Rule('Убираем окончания числительных', function (text) {
                    return  text
                      .replace(new RegExp('(\\d)-я(?=(\\s+[^\\s]+(?:-я|ая|ья|яя|яся))*\\s+(улица|набережная|дорога|линия|Линия|аллея|площадь|просека|автодорога|эстакада|магистраль|дамба|хорда|коса|деревня|переправа|площадка|дорожка|трасса|Новостройка|Лесоперевалка|Ферма|Авеню|Пристань|Рота|Слобода)(\\s|$))', 'g'), '$1')
                      .replace(new RegExp('(\\d)-й(?=(\\s+[^\\s]+(?:[-иоы]й|ин|[оеё]в))*\\s+(проспект|переулок|проезд|тупик|бульвар|тракт|просек|взвоз|спуск|разъезд|переезд|квартал|Квартал|путепровод|мост|сад|сквер|тоннель|туннель|парк|проток|канал|остров|микрорайон|район|городок|посёлок|поселок|вал|проулок|Поляны|Пятилетки|Участок)(\\s|$))', 'g'), '$1');

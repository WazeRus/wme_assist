            ...
            return rules_basicRU().concat([
                ...
                new Rule('Убираем окончания числительных', function (text) {
                    return  text
                      ...
                      .replace(new RegExp('(\\d)-й(?=(\\s+[^\\s]+(?:[-иоы]й|ин|[оеё]в))*\\s+(проспект|переулок|проезд|тупик|бульвар|тракт|просек|взвоз|спуск|разъезд|переезд|квартал|Квартал|путепровод|мост|сад|сквер|тоннель|туннель|парк|проток|канал|остров|микрорайон|район|городок|посёлок|поселок|вал|проулок|Поляны|Пятилетки)(\\s|$))', 'g'), '$1');
